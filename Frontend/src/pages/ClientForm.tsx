...
function ClientForm() {
  ...
  const loadClient = async (clientId: number) => {
    try {
      ...
      toastError('Erro ao carregar cliente.');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditMode && id) {
        await clientService.update(parseInt(id, 10), dataToSend);
      ...
      if (Array.isArray(detail)) {
        ...
        const msg = isEditMode ? 'Erro ao atualizar cliente' : 'Erro ao criar cliente';
        setError(msg);
        toastError(msg);
      }
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === 'cnpj') {
      ...
      maskedValue = numbers ? numbers.replace(/^(\d{5})(\d)/, '$1-$2') : '';
  ...
  const handleLogoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('image/')) {
      toastError('Selecione uma imagem válida (JPG, PNG, GIF).');
      return;
    }
    ...
    <div className="client-form-page">
      <div className="client-form-container">
        ...
        <form onSubmit={handleSubmit}>
          ...
          {/* Mensagem de erro opcional (além do toast) */}
